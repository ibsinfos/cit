...
{
const VIEW_FOLDER = 'admin/aboutus';
public function __construct()
{
	parent::__construct();
	$this->load->model('admin/aboutus_model');
...
public function index()
{
	...
	$config['base_url'] = base_url().'admin/aboutus';
	...
	$data['order_type_selected'] = $order_type;
        if($search_string !== false && $order !== false || $this->uri->segment(3) == true)
		{
            ...
            $data['count_products']= $this->aboutus_model->count_aboutus($search_string, $order);
            $config['total_rows'] = $data['count_products'];
            if($search_string)
			{
                if($order){
                    $data['aboutus'] = $this->aboutus_model->get_aboutus($search_string, $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['aboutus'] = $this->aboutus_model->get_aboutus($search_string, '', $order_type, $config['per_page'],$limit_end);           
                }
            }else
			{
                if($order)
				{
                    $data['aboutus'] = $this->aboutus_model->get_aboutus('', $order, $order_type, $config['per_page'],$limit_end);        
                }
				else
				{
                    $data['aboutus'] = $this->aboutus_model->get_aboutus('', '', $order_type, $config['per_page'],$limit_end);        
                }
            }
		...
		else
		{
            ...
            $data['order'] = 'id';
            $data['count_products']= $this->aboutus_model->count_aboutus();
            $data['aboutus'] = $this->aboutus_model->get_aboutus('', '', $order_type, $config['per_page'],$limit_end);        
            $config['total_rows'] = $data['count_products'];
...
public function add()
{
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
			...
			if($this->form_validation->run() == TRUE)
			{
                ...
                if($this->aboutus_model->store_aboutus($data_to_store))
				{
...
public function update()
{
	...
	{
		$image=$this->input->post('old_aboutus_image');
	}
		$data_to_store = array(
					...
					'aboutus_image' => $image,
					'aboutus_des' => $this->input->post('aboutus_des'),	
					'aboutus_des1' => $this->input->post('aboutus_des1'),
					);
		if($this->aboutus_model->update_aboutus($id, $data_to_store) == TRUE)
		{
...
public function details()
{
	...
	$data['aboutus'] = $this->aboutus_model->get_aboutus_by_id($id);
	$data['main_content'] = 'admin/aboutus/details';
	$this->load->view('includes/header', $data);  
	$this->load->view('includes/left_sidebar', $data);
	$this->load->view('admin/aboutus/details', $data);
	$this->load->view('includes/footer', $data);
}
