...
public function index()
{
	...
	$config['num_links'] =$this->faqs_model->count_faqs();
	...
	$data['order_type_selected'] = $order_type;
        if($search_string !== false && $order !== false || $this->uri->segment(3) == true)
		{
            ...
            $data['count_products']= $this->faqs_model->count_faqs($search_string, $order);
            $config['total_rows'] = $data['count_products'];
            if($search_string)
			{
                if($order){
                    $data['faqs'] = $this->faqs_model->get_faqs($search_string, $order, $order_type, $config['per_page'],$limit_end);        
                }else{
                    $data['faqs'] = $this->faqs_model->get_faqs($search_string, '', $order_type, $config['per_page'],$limit_end);           
                }
            }else
			{
                if($order)
				{
                    $data['faqs'] = $this->faqs_model->get_faqs('', $order, $order_type, $config['per_page'],$limit_end);        
                }
				else
				{
                    $data['faqs'] = $this->faqs_model->get_faqs('', '', $order_type, $config['per_page'],$limit_end);        
                }
            }
		...
		else
		{
            ...
            $data['order'] = 'id';
            $data['count_products']= $this->faqs_model->count_faqs();
            $data['faqs'] = $this->faqs_model->get_faqs('', '', $order_type, $config['per_page'],$limit_end);        
            $config['total_rows'] = $data['count_products'];
	...
	$data['main_content'] = 'admin/faqs/list';
...
public function add()
{
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST')
        {
			...
			if($this->form_validation->run() == TRUE)
			{
                ...
                if($this->faqs_model->store_faqs($data_to_store))
				{
        ...
        $data['main_content'] = 'admin/faqs/add';
		$this->load->view('includes/header', $data);  
		$this->load->view('includes/left_sidebar', $data);
...
public function update()
{
	...
	if ($this->input->server('REQUEST_METHOD') === 'POST')
	{
		...
		if($this->faqs_model->update_faqs($id, $data_to_store) == TRUE)
		{
		...
		redirect('admin/faqs/update/'.$id.'');
	}
	$data['faqs'] = $this->faqs_model->get_faqs_by_id($id);
	$data['main_content'] = 'admin/faqs/edit';
	$this->load->view('includes/header', $data);  
	$this->load->view('includes/left_sidebar', $data);
...
public function details()
{
	...
	$data['faqs'] = $this->faqs_model->get_faqs_by_id($id);
	$data['main_content'] = 'admin/faqs/details';
	$this->load->view('includes/header', $data);  
	$this->load->view('includes/left_sidebar', $data);
	$this->load->view('admin/faqs/details', $data);
	$this->load->view('includes/footer', $data);
}
