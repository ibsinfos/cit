...
<link rel="stylesheet" href="<?php echo base_url(); ?>css/jquery.mCustomScrollbar.css">
	...
	<div class="kf_content_wrap">
					...
					<div class="row">
						...
						<div class="col-md-8">
							<div class="row">
								...
								<div class="kf_cur_catg_wrap" style="padding-top:0px;">
								...
								<form method="post" action="<?php echo base_url(); ?>home/course_inquiry" >
									...
									<div class="edu2_serc_des">
										<select name="ci_cat" >
											...
											<?php 
											   $sql_cour=$this->db->query("SELECT course_name FROM `courses`");
											   foreach($sql_cour->result() as $rowci){
											 ?>
                                            <option value="<?php echo $rowci->course_name; ?>" <?php if($ci_cat==$rowci->course_name) echo 'selected'; ?> ><?php echo $rowci->course_name; ?></option>
											<?php } ?>
									...
									<div class="edu2_serc_des">
										<textarea rows="5" name="ci_des" class="form-control" placeholder="More Details"><?php
			...
			<!--KF COURSES CATEGORIES WRAP END-->
			 ...
			 <?php 
			     $sqlcount=$this->db->query("SELECT * FROM `homefollow` WHERE `homefollow_id`=1");
				 $row_ff=$sqlcount->row();
			  ?>
			...
			<section class="edu2_counter_wrap">
				<div class="container">
					...
					<div class="edu2_counter_des">
						...
						<h3 class="counter"><?php echo $row_ff->ff_count; ?></h3>
						<h5><?php echo $row_ff->title1; ?></h5>
					</div>
					<!--EDU2 COUNTER DES END-->
					<!--EDU2 COUNTER DES START-->
					<div class="edu2_counter_des">
						<span><i class="icon-book236"></i></span>
						<h3 class="counter"><?php echo $row_ff->cc_count; ?></h3>
						<h5><?php echo $row_ff->title2; ?></h5>
					</div>
					<!--EDU2 COUNTER DES END-->
					<!--EDU2 COUNTER DES START-->
					<div class="edu2_counter_des">
						<span><i class="icon-win5"></i></span>
						<h3 class="counter"><?php echo $row_ff->se_count; ?></h3>
						<h5><?php echo $row_ff->title3; ?></h5>
					</div>
					<!--EDU2 COUNTER DES END-->
					<!--EDU2 COUNTER DES START-->
					<div class="edu2_counter_des">
						<span><i class=" icon-user255"></i></span>
						<h3 class="counter"><?php echo $row_ff->ct_count; ?></h3>
						<h5><?php echo $row_ff->title4; ?></h5>
					</div>
					<!--EDU2 COUNTER DES END-->
				</div>
			</section>
			...
			<!-- FACULTY WRAP START-->
			<?php if($res_setting->faculty=='yes'){ ?>
			<section>
				<div class="container">
					<div class="row">
						<!-- HEADING 1 START-->
			...
			<section class="edu2_tarining_bg">
			  ...
			  <?php } ?><?php */?>
              <section class="edu2_tarining_bg" style="padding:0px;">            
	<div id="owl-demo-23">
	<?php 
			$sql_c=$this->db->query("SELECT * FROM `homecourse` WHERE cdisplay='yes' ");
			 
		foreach($sql_c->result() as $rec_f)
		{
			  if($rec_f->cdisplay=='yes')
			  {
				date_default_timezone_set('Asia/kolkata');  
		...
		<img src="images/<?php echo $rec_f->bg_image; ?>" alt="" />
        	<div class="container trapospl">
					<div class="row">
						<div class="col-md-4">
							<div class="kf_edu2_training_des">
								<figure>
									<img src="images/<?php echo $rec_f->h_image; ?>" alt=""/>
								</figure>
							</div>
						</div>
...
<?php if($res_setting->gallery=='yes'){ ?>
